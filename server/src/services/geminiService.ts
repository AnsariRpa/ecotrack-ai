 ...
 */
export async function generateExplainableRecommendations(
  activities: Activity[]
  ...
  try {
    ...
    const parsed: ExplainableRecommendation[] = JSON.parse(jsonText.trim());
...
function buildActivitySummary(activities: Activity[]) {
  const catTotals: Record<string, { kgCO2: number; count: number }> = {
...
function buildCoachPrompt(summary: ReturnType<typeof buildActivitySummary>): string {
 ...
 */
export async function generateCarbonForecast(
  activities: Activity[],
  forecastDays: 7 | 30 = 7,
  targetKgCO2PerDay?: number
  ...
  try {
    ...
    const parsed: CarbonForecast = JSON.parse(jsonText.trim());
...
function buildHistoricalTimeSeries(activities: Activity[]) {
...
function buildForecastPrompt(
  history: { date: string; kgCO2: number }[],
  forecastDays: number,
  targetKgCO2PerDay?: number
): string {
  ...
  const avgDaily =
    history.length > 0 ? history.reduce((s, h) => s + h.kgCO2, 0) / history.length : 8.0;
...
function generateFallbackRecommendations(activities: Activity[]): ExplainableRecommendation[] {
  const recommendations: ExplainableRecommendation[] = [];

  const catTotals: Record<string, number> = { transportation: 0, food: 0, energy: 0, waste: 0 };
  activities.forEach((a) => {
    if (catTotals[a.category] !== undefined) catTotals[a.category] += a.carbonTons;
  });
  ...
  if (carKm > 50) {
    recommendations.push({
      ...
      reasoning:
        "Your car usage is significantly above the sustainable mobility threshold of 50 km/week.",
      behaviourInsight:
        "High single-occupancy vehicle use suggests routine car commuting or errands by car.",
      reductionMethod:
        "Use bus, metro, or tram for regular routes; reserve car for cargo-heavy or accessibility needs.",
      sustainabilityImpact:
        "Reduced particulate matter and NOx emissions improve urban air quality and public health.",
      actionableSteps: [
  ...
  if (beefCount >= 2) {
    recommendations.push({
      ...
      behaviourInsight:
        "Regular beef consumption is the single highest-impact dietary carbon factor.",
      reductionMethod:
        "Substitute with lentils, chickpeas, tofu, or mycoprotein alternatives like Quorn.",
      sustainabilityImpact:
        "Reduces methane emissions from livestock and land use for feed crops, protecting biodiversity.",
      actionableSteps: [
  ...
  if (elecKwh > 30) {
    recommendations.push({
      ...
      reductionMethod:
        "Install smart power strips or use scheduled outlet timers for entertainment systems.",
      sustainabilityImpact:
        "Reduced peak demand lowers reliance on gas peaker plants that spike grid carbon intensity.",
      actionableSteps: [
  ...
  if (generalWaste > 3) {
    recommendations.push({
      ...
      reasoning:
        "High general waste volume indicates mixed disposal — recyclables and organics are going to landfill.",
      behaviourInsight:
        "Most households dispose of 40-60% recyclable or compostable material as general waste.",
      reductionMethod:
        "Introduce a kitchen compost bin for food scraps and dedicated containers for plastic, glass, paper.",
      sustainabilityImpact:
        "Composting diverts organic methane from landfill; recycling reduces virgin material extraction.",
      actionableSteps: [
  ...
  const DEFAULTS: ExplainableRecommendation[] = [
    {
      category: "transportation",
      title: "Walk or Cycle for Trips Under 3 km",
      content:
        "For short local trips, active travel produces zero emissions, saves fuel costs, and improves cardiovascular health.",
      potentialSaving: 2.5,
    ...
    },
    {
      ...
      content:
        "Plant-based meals produce up to 10× fewer emissions than beef. Even one meal swap per week compounds into meaningful annual savings.",
      potentialSaving: 4.2,
    ...
    },
    {
      ...
      content:
        "LED bulbs use 75% less energy than incandescent and last 25× longer. Replacing 10 bulbs can save 200 kWh/year.",
      potentialSaving: 3.8,
    ...
    },
    {
      ...
      content:
        "Food waste in landfill generates methane — 28× more potent than CO₂ over 100 years. A countertop compost bin takes 30 seconds to set up.",
      potentialSaving: 1.8,
...
function generateFallbackForecast(
  ...
  const dailyBreakdown: DailyForecast[] = Array.from({ length: forecastDays }, (_, i) => {
    const date = new Date();
  ...
  return {
    ...
    trend,
    insights: [
      `Your average daily emission is ${avgDaily.toFixed(1)} kg CO₂ — ${
        avgDaily < 6 ? "below" : avgDaily < 10 ? "near" : "above"
      } the global average of 8 kg/day.`,
      trend === "improving"
