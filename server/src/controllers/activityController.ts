...
async function checkAndUnlockAchievements(userId: string): Promise<string[]> {
  ...
  const plantMealsCount = activities.filter(
    (act) => act.category === "food" && (act.type === "vegan" || act.type === "vegetarian")
  ).length;
...
export async function getDashboardSummary(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    let rating = "A";
    if (dailyAverage7Days > 12) rating = "B";
    if (dailyAverage7Days > 22) rating = "C";
    if (dailyAverage7Days > 35) rating = "D";
    if (dailyAverage7Days > 50) rating = "E";
