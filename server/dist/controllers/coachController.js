...
export async function getCoachingAdvice(req, res, next) {
    try {
        // Resolve userId from query param, auth context, or default user
        let finalUserId = (req.query.userId ?? req.user?.uid);
        ...
        const savedRecommendations = await Promise.all(recommendations.map((rec) => prisma.recommendation.create({
            data: {
                userId: finalUserId,
